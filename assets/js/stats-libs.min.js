/**
 * stats-libs.min.js - 第三方依赖与 Polyfill（ECharts/WordCloud2 配置、资源加载、eval5 错误屏蔽）
 * 保持原样，不进行业务逻辑修改。IIFE 顶部声明 _loc 以统一规范。
 */
(function() {
    'use strict';
    ...
    function loadScript(url, timeout) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            script.async = false;
            var isDone = false;
            script.onload = function() { isDone = true; console.log('[📦] 加载成功:', url); resolve(url); };
    ...
    function isDeployedOrigin() {
        var h = _loc.hostname || '';
        return h.endsWith('.github.io') || h.endsWith('.pages.dev') || h.endsWith('.vercel.app') || h.endsWith('.netlify.app');
    }

    function tryCdn(name, config) {
    ...
    function loadResource(name, config) {
        var tryLocal = config.local && !isDeployedOrigin();
        if (tryLocal) {
            return loadScript(config.local, 5000).then(function() {
                console.log('[✅] ' + name + ': 本地资源');
                return { source: 'local' };
            }).catch(function() {
                return tryCdn(name, config);
            }).catch(function(err) {
                throw new Error(name + ' 加载失败: ' + (err && err.message));
            });
        }
        return tryCdn(name, config).then(function(r) { return r; }, function(err) {
            throw new Error(name + ' 加载失败: ' + (err && err.message));
        });
    }

    window._resourceLoader = function() {
        console.group('[🚀] 加载资源 (离线优先)');
        return loadResource('echarts', RESOURCE_CONFIG.echarts).then(function() {
            if (typeof echarts !== 'undefined' && echarts.registerLocale) {
                var zhLocale = {
                    ...
                    series: {
                        typeNames: {
                            pie: '饼图', bar: '柱状图', line: '折线图', scatter: '散点图', effectScatter: '涟漪散点图', radar: '雷达图',
                            tree: '树图', treemap: '矩形树图', boxplot: '箱型图', candlestick: 'K线图', k: 'K线图', heatmap: '热力图',
                            map: '地图', parallel: '平行坐标图', lines: '线图', graph: '关系图', sankey: '桑基图', funnel: '漏斗图',
                            gauge: '仪表盘图', pictorialBar: '象形柱图', themeRiver: '主题河流图', sunburst: '旭日图'
                        }
                    },
                    aria: {
                        ...
                        data: {
                            allData: '其数据是——', partialData: '其中,前{displayCnt}项是——',
                            withName: '{name}的数据是{value}', withoutName: '{value}',
                            separator: { middle: ',', end: '' }
                        }
                    }
                };

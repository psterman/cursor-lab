-- ============================================
-- 升级：get_user_ranks_6d 本国排名按 current_location 分区
-- 逻辑：用户是 CN 时应在 current_location='CN' 的子集中计算本国排名，不再用 ip_location
-- loc = current_location || manual_location || ip_location || 'XX'
-- ============================================
...
COMMENT ON FUNCTION public.get_user_ranks_6d IS '返回 6 维度双排名（本国按 current_location 分区），供 /api/country-summary 使用';

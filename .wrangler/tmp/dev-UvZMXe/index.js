...
// .wrangler/tmp/bundle-FrAgID/checked-fetch.js
...
app.post("/api/v2/analyze", async (c) => {
  try {
    ...
    console.log("[Worker] \u{1F4CA} \u7EF4\u5EA6\u8BA1\u7B97\u7ED3\u679C:", {
      L: dimensions.L,
      P: dimensions.P,
      D: dimensions.D,
      E: dimensions.E,
    ...
    if (env.SUPABASE_URL && env.SUPABASE_KEY) {
      try {
        ...
        console.log("[Worker] \u{1F4E5} Supabase \u54CD\u5E94\u72B6\u6001:", {
          ok: writeRes.ok,
    ...
    if (env.SUPABASE_URL && env.SUPABASE_KEY) {
      try {
        ...
        if (totalUsers > 1) {
          ...
          ranks = {
            messageRank: calcPct(beatL),
            // 消息数排名（用 L 维度）
            charRank: calcPct(beatP),
            // 字符数排名（用 P 维度）
...
// .wrangler/tmp/bundle-FrAgID/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-FrAgID/middleware-loader.entry.ts

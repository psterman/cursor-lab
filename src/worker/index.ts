...
app.get('/api/country-summary', async (c) => {
  try {
    ...
    // ----------------------------
    // ✅ 新方案：一次 RPC 返回“国家总额 + 排名”
    // Supabase 禁止对视图做 sum()/avg() 时，必须走数据库函数
    // ----------------------------
    ...
    // total_chars 的排名：若 RPC 未提供，退回使用 rank_user_chars（两者在现有口径常一致）
    const rankUserChars = row?.rank_user_chars ?? row?.rank_total_user_chars;
    const countryTotalsRanks: any = {
...
async function handleWebSocketProxy(request: Request, env: Env): Promise<Response> {
  ...
  const realtimePath = url.pathname; // 例如: /realtime/v1
  const targetUrl = `${supabaseUrl.replace(/^https?:\/\//, 'wss://')}${realtimePath}`;
  ...
  try {
    ...
    await clientSocket.accept();

    // Supabase -> Client
    serverPair.addEventListener('message', (event) => {
      try {
    ...
    serverPair.addEventListener('close', (event) => {
    ...
    serverPair.addEventListener('error', (error) => {
      console.error('[WS Proxy] Server error:', error);
      try {
        clientSocket.close(1011, 'Proxy server error');
      } catch (e) {
        // Ignore
      }
    });
    ...
    clientSocket.addEventListener('message', (event) => {
      try {
        if (serverPair.readyState === WebSocket.OPEN) {
          serverPair.send(event.data);
        }
      } catch (e) {
    ...
    clientSocket.addEventListener('close', (event) => {
      ...
      if (serverPair.readyState === WebSocket.OPEN) {
        serverPair.close(event.code, event.reason);
      }
    });
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // 检查是否为 WebSocket 升级请求且路径为 realtime 相关

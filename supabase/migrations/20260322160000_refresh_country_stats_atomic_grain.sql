-- ============================================
-- 国家级原子颗粒对齐：DDL + refresh_country_stats_current + v_country_stats_rollup
-- cursor_total_messages_sum = SUM(col_total_messages)，col 优先 total_messages 列，回退 stats.cursor.total_messages
--
-- 手动刷新（必须用 SELECT，否则报 syntax error near "refresh_country_stats_current"）：
--   SELECT public.refresh_country_stats_current();
-- 错误示例：refresh_country_stats_current();  -- PostgreSQL 不允许单独调用函数语句
--
-- 抽样：SELECT country_code, cursor_total_messages_sum, github_total_repos_sum, main_language_mode FROM public.country_stats_current LIMIT 10;
-- ============================================

alter table public.country_stats_current
  add column if not exists main_language_mode text,
  add column if not exists main_language_mode_users bigint not null default 0,
  add column if not exists cursor_total_messages_sum bigint not null default 0,
  add column if not exists github_total_repos_sum bigint not null default 0;
...
comment on column public.country_stats_current.main_language_mode is 'github_stats 主语言众数（primaryLanguage / mainLanguage）';
comment on column public.country_stats_current.main_language_mode_users is '使用该众数语言的用户数';
comment on column public.country_stats_current.cursor_total_messages_sum is '赛博磕头：SUM(col_total_messages)，列 total_messages 优先，回退 stats.cursor.total_messages';
comment on column public.country_stats_current.github_total_repos_sum is '赛博仓鼠：SUM(totalRepos 或 publicRepos+privateRepos)';
comment on column public.country_stats_current.cursor_metrics is 'Cursor 原始指标聚合（messages/total_chars/work_days）';
comment on column public.country_stats_current.lobster_metrics is 'OpenClaw 原始指标聚合';
...
begin
  with base as (
    select
      ...
      coalesce(
        nullif((stats->'cursor'->>'total_messages'), '')::bigint,
        coalesce(total_messages, 0)::bigint,
        0::bigint
      ) as col_total_messages,
      case
  ...
  src as (
    select
      ...
      sum(b.col_total_messages)::bigint as cursor_total_messages_sum,
...
'国家级聚合：total_messages_sum=cursor+openclaw；cursor_total_messages_sum=SUM(col_total_messages)；rank_score=cursor_total_messages_sum';
...
comment on view public.v_country_stats_rollup is '国家级信仰榜指标与人均（来源 country_stats_current）';

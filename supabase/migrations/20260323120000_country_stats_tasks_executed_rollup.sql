-- 全球硬核榜：lobster_metrics.tasks_executed（OpenClaw stats JSON，缺省为 0）；扩展 v_country_stats_rollup 透出 jiafang / tool_calls / tasks
...
begin
  with base as (
    select
      ...
      coalesce(nullif((stats->'openclaw'->'stats'->>'tasks_executed'), '')::bigint, 0::bigint) as openclaw_tasks_executed,
...
comment on column public.country_stats_current.lobster_metrics is 'OpenClaw：messages, tool_calls, tasks_executed（stats.openclaw.stats.tasks_executed SUM）, total_chars, work_days';

-- 插入新列时不能用 CREATE OR REPLACE（会触发 42P16 列名/位置错位）；先 DROP 再 CREATE
...
select
  ...
  (jiafang_count_sum::numeric / nullif(cursor_total_messages_sum, 0)::numeric) as jiafang_rejection_rate

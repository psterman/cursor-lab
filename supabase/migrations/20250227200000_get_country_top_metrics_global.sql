-- 支持 get_country_top_metrics_v1(country_code := null) 表示全球榜（不按国家过滤）
-- 原逻辑 WHERE u.country_code = (SELECT cc FROM params) 在 cc 为 null 时永远不匹配
...
base AS (
  ...
  WHERE (SELECT cc_raw FROM params) IS NULL
     OR UPPER(TRIM(u.country_code)) = UPPER(TRIM((SELECT cc_raw FROM params)))
),
tm_rows AS (
  SELECT * FROM base
...
tm_top1 AS (
  SELECT total_messages AS score,
    jsonb_build_object(
      'id', id,
      'user_name', user_name,
      'github_username', github_username,
...
tuc_top1 AS (
  SELECT total_user_chars AS score,
    jsonb_build_object(
      'id', id,
      'user_name', user_name,
      'github_username', github_username,
...
avg_top1 AS (
  SELECT avg_user_message_length AS score,
    jsonb_build_object(
      'id', id,
      'user_name', user_name,
      'github_username', github_username,
...
'某国家或全球 6 指标 TopN 榜单；country_code 为 null 或空时返回全球榜。';

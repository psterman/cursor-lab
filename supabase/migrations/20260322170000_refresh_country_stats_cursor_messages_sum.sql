-- ============================================
-- 对齐：cursor_total_messages_sum = SUM(各用户 cursor_messages)，去掉冗余 col_total_messages
-- 部署后：SELECT public.refresh_country_stats_current();
-- 验收：cursor_total_messages_sum 应等于该国所有 user_analysis 行 Base.cursor_messages 之和
-- ============================================

comment on column public.country_stats_current.cursor_total_messages_sum is
  '赛博磕头：各国用户 cursor_messages 行级值之和（stats.cursor.total_messages 与 total_messages 列回退，与 cursor_metrics.messages 同源）';
...
'国家级聚合：total_messages_sum=cursor+openclaw；cursor_total_messages_sum=SUM(cursor_messages)；rank_score=cursor_total_messages_sum';

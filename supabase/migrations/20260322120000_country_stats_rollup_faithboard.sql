-- ============================================
-- 国家大盘（信仰榜）预聚合：mainLanguage 众数、Cursor total_messages SUM、GitHub totalRepos SUM
-- rank_score 改为纯 SUM(cursor_total_messages_sum)，不再使用加权公式
-- ============================================
...
comment on column public.country_stats_current.main_language_mode is 'github_stats mainLanguage/primaryLanguage 众数';
comment on column public.country_stats_current.main_language_mode_users is '使用众数语言的用户数';
...
'刷新 country_stats_current：双源 cursor/openclaw；rank_score=cursor_total_messages_sum（SUM）；信仰榜字段 main_language_* / github_total_repos_sum';
...
comment on view public.v_country_stats_rollup is '国家级信仰榜/仓鼠/磕头预聚合与人均（来自 country_stats_current）';

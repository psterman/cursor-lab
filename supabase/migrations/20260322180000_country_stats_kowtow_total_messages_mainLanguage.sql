-- ============================================
-- 赛博磕头：SUM(user_analysis.total_messages) 物理列
-- 最夯语言：main_lang_raw 优先 github_stats.mainLanguage（众数不变）
-- 赛博仓鼠：仅 github_stats.totalRepos（可解析为整数时），否则 0
-- 部署后：SELECT public.refresh_country_stats_current();
-- ============================================

comment on column public.country_stats_current.cursor_total_messages_sum is
  '赛博磕头：SUM(total_messages) 列（Cursor 对话回合物理字段）';

comment on column public.country_stats_current.github_total_repos_sum is
  '赛博仓鼠：优先 SUM(totalRepos)；缺省时为 publicRepos+privateRepos（与同步服务写入一致）';
...
begin
  with base as (
    select
      ...
      coalesce(total_messages, 0)::bigint as col_total_messages,
      case
...
'cursor_total_messages_sum=SUM(total_messages)；github_total_repos_sum=SUM(totalRepos)；main_language_mode=mainLanguage 众数';

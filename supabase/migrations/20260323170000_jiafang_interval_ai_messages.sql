-- 杠精榜：jiafang_rejection_rate 改为「约多少条 AI 回复 / 次否定信号」= SUM(ai_messages) / SUM(jiafang_count)
-- ai_messages 取自 stats.cursor.ai_messages / aiMessages；缺失时用 Cursor 对话条数 /2 近似（用户/助手轮次）
...
comment on column public.country_stats_current.cursor_metrics is 'Cursor：messages, ai_messages（SUM AI 回复条数）, total_chars, work_days';
...
comment on view public.v_country_stats_rollup is '国家级聚合；jiafang_rejection_rate = SUM(ai_messages)/SUM(jiafang_count)，表示约多少条 AI 回复对应 1 次甲方化/否定信号';
